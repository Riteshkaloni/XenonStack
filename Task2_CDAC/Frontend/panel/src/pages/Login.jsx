    ...
    const submit = async () => {
        ...
        if (phone == "" || password == "") {
            ...
            loginApi("login", {
                ...
                console.log(result)
                if (result.error == 0) {
                    localStorage.setItem(Constants.APP_KEY, result.token)
                    localStorage.setItem(Constants.USERNAME, result.name)
                    localStorage.setItem(Constants.USERTYPE, "SA")
                    setGlobalAccess(true)
                    setGlobalAccessType("SA")
                    navigate("/admin_list")
                    ...
                    <div class="h-screen xl:h-auto flex py-5 xl:py-0 my-10 xl:my-0">
                        <div class="my-auto mx-auto xl:ml-20 bg-white dark:bg-darkmode-600 xl:bg-transparent px-5 sm:px-8 py-8 xl:p-0 rounded-md shadow-md xl:shadow-none w-full sm:w-3/4 lg:w-2/4 xl:w-auto">
                            ...
                            <div class="intro-x mt-5 xl:mt-8 text-center xl:text-left">
                                <button onClick={() => submit()} class="btn btn-primary py-3 px-4 w-full xl:w-32 xl:mr-3 align-top">Login</button>
                            </div>
                            <div class="intro-x mt-10 xl:mt-24 text-slate-600 dark:text-slate-500 text-center xl:text-left"> Not a user <a class="text-primary dark:text-slate-200 underline" href="/register">Signup</a>  <a class="text-primary dark:text-slate-200" href=""></a> </div>
                        </div>
                    </div>
                </div>
            </div>
